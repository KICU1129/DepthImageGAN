    ...
    for i, batch in enumerate(dataloader):
        ...
        if i % 20 == 0:
            print('Epoch[{}]({}/{}) loss_G: {:.4f} loss_G_identity: {:.4f} loss_G_GAN: {:.4f} loss_G_cycle: {:.4f} loss_D: {:.4f}'.format(
                epoch, i, len(dataloader), loss_G, (loss_identity_B),
                (loss_GAN_A2B ), (0), (loss_D_B)
                ))
        ...
        train_info = {
            ...
            'batch_num': i, 
            'lossG': loss_G.item(),
            'lossG_identity': (loss_identity_B.item()),
            'lossG_GAN': (loss_GAN_A2B.item() ),
            'lossG_cycle': (0),
            'lossD': (loss_D_B.item()), 
            }
...
#Save generate image from netG
image_pathes=recorder.save_image(netG_A2B,None,sample_images,input_A,input_B=input_B)
