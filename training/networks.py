...
if __name__=="__main__":
    ...
    netG=define_G(input_nc=3,output_nc=1,ngf=64,netG='resnet_9blocks').to(device)
    netD=define_D(input_nc=1,ndf=64,netD="basic").to(device)

    dummy_input_G=torch.randn(1,3,640,480).to(device)#ダミーの入力を用意する
    dummy_input_D=torch.randn(1,1,640,480).to(device)#ダミーの入力を用意する
    input_names = [ "input"]
    output_names = [ "output" ]
